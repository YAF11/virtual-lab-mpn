(() => {
  ...
  const state = { stage: 1, inoculated: new Set(), positives: new Set(), selectedPositives: new Set(), sourceTube: null, sourceBglb: null, bglbTube: null, ecbTube: null, bglbTubes: new Set(), ecbTubes: new Set(), usedLTBForBGLB: new Set(), usedLTBForECB: new Set(), alcoholSprayed: false, oseSterile: false, oseLoaded: false, ltbInIncubator: false, bglbInIncubator: false, ecbInIncubator: false, timer: null, mpnValue: '' };
  const steps = [
    ...
    ['ECB (jika diperlukan)', 'Konfirmasi lanjutan dengan ECB', 'Pijarkan ose, ambil inokulum dari LB positif, lalu masukkan ke ECB. Setelah itu masukkan rak ECB ke inkubator 44°C.'],
  ...
  function updateResults() {
    ...
    if (state.stage === 6) content = resultsMarkup(`<p>Transfer LTB positif ke BGLB: <b>${state.bglbTubes.size}/6</b> tabung.</p>`);
    if (state.stage === 7) content = resultsMarkup(`<p>Transfer LB positif ke ECB: <b>${state.ecbTubes.size}/6</b> tabung.</p><p>Hasil BGLB tidak menunjukkan gas. Konfirmasi <i>E. coli</i> tetap mengikuti SOP laboratorium.</p>`);
  ...
  function setStage(stage) {
    ...
    if (stage === 6) { state.alcoholSprayed = false; alcohol.classList.remove('sprayed'); requireObjects(alcohol, bunsen, ose, bglbRack, ...$$('.test-tube.positive')); setStatus('Semprot alkohol 70% pada awal pengerjaan BGLB, pijarkan ose, lalu pilih tabung LTB positif.'); }
    if (stage === 7) { state.alcoholSprayed = false; state.oseSterile = false; state.oseLoaded = false; alcohol.classList.remove('sprayed'); ose.classList.remove('glowing', 'loaded'); requireObjects(alcohol, bunsen, ose, ecbRack, incubator44, ...$$('.test-tube.positive')); setStatus('Semprot alkohol 70% pada awal pengerjaan ECB, pijarkan ose, ambil LB positif, lalu masukkan ke ECB.'); }
  ...
  function handleDrop(object, target) {
    ...
    if (state.stage === 6 && object === bglbRack) { if (state.bglbTubes.size < 6) { setStatus(`✕ Masukkan seluruh 6 inokulum LB positif ke BGLB terlebih dahulu (${state.bglbTubes.size}/6).`, 'alert'); return; } if (target === incubator) { state.bglbInIncubator = true; setStatus('✓ Rak BGLB masuk ke inkubator 37°C.', 'positive-note'); setAction('Mulai inkubasi BGLB • 2 × 24 jam', incubateBGLB); } else setStatus('✕ Masukkan rak BGLB ke inkubator 37°C.', 'alert'); return; }
    if (state.stage === 6 && object === ose) {
      ...
      if (tube) { if (!state.oseLoaded) { setStatus('✕ Ambil inokulum LB positif dengan ose yang sudah dipijarkan.', 'alert'); return; } if (state.bglbTubes.has(tube)) { setStatus('Tabung BGLB ini sudah diinokulasi.', 'alert'); return; } state.bglbTube = tube; state.bglbTubes.add(tube); state.usedLTBForBGLB.add(state.sourceTube); tube.classList.add('inoculated'); animateLiquid(tube); state.oseLoaded = false; ose.classList.remove('loaded'); state.sourceTube = null; updateResults(); if (state.bglbTubes.size === 6) { setStatus('✓ Enam BGLB telah diinokulasi. Masukkan rak BGLB ke inkubator 37°C.', 'positive-note'); requireObjects(bglbRack, incubator); } else { setStatus(`✓ Inokulum masuk ke BGLB (${state.bglbTubes.size}/6). Pilih LB positif berikutnya, pijarkan ose, lalu ulangi.`, 'positive-note'); requireObjects(bunsen, ose, ...$$('.test-tube.positive').filter(t => !state.usedLTBForBGLB.has(+t.dataset.index))); } return; }
      setStatus('✕ Arahkan ose ke Bunsen, LTB positif, atau salah satu tabung BGLB.', 'alert'); return;
    }
    if (state.stage === 7 && object === ecbRack) { if (state.ecbTubes.size < 6) { setStatus(`✕ Masukkan seluruh 6 inokulum LB positif ke ECB terlebih dahulu (${state.ecbTubes.size}/6).`, 'alert'); return; } if (target === incubator44) { state.ecbInIncubator = true; setStatus('✓ Rak ECB masuk ke inkubator 44°C.', 'positive-note'); setAction('Mulai inkubasi ECB • 2 × 24 jam', incubateECB); } else setStatus('✕ Masukkan rak ECB ke inkubator 44°C.', 'alert'); return; }
    if (state.stage === 7 && object === ose) {
      if (target === bunsen) { if (!state.alcoholSprayed) { setStatus('✕ Semprot alkohol 70% terlebih dahulu.', 'alert'); return; } state.oseSterile = true; state.oseLoaded = false; ose.classList.remove('loaded'); ose.classList.add('glowing'); setStatus('✓ Ose berpijar. Ambil inokulum dari LB positif.', 'positive-note'); return; }
      const ltb = target?.closest?.('.test-tube');
      if (ltb) { const index = +ltb.dataset.index; if (!state.oseSterile) { setStatus('✕ Pijarkan ose pada Bunsen terlebih dahulu.', 'alert'); return; } if (state.sourceTube !== index) { setStatus('✕ Angkat/pilih tabung LB positif terlebih dahulu.', 'alert'); return; } state.oseSterile = false; state.oseLoaded = true; ose.classList.remove('glowing'); ose.classList.add('loaded'); setStatus('✓ Inokulum LB diambil dengan ose. Masukkan ke ECB.', 'positive-note'); return; }
      const ecb = target?.closest?.('.ecb-tube');
      if (ecb) { if (!state.oseLoaded) { setStatus('✕ Ambil inokulum LB positif dengan ose terlebih dahulu.', 'alert'); return; } if (state.ecbTubes.has(ecb)) { setStatus('Tabung ECB ini sudah diinokulasi.', 'alert'); return; } state.ecbTube = ecb; state.ecbTubes.add(ecb); state.usedLTBForECB.add(state.sourceTube); ecb.classList.add('inoculated'); animateLiquid(ecb); state.oseLoaded = false; ose.classList.remove('loaded'); state.sourceTube = null; updateResults(); if (state.ecbTubes.size === 6) { setStatus('✓ Enam ECB telah diinokulasi. Masukkan rak ECB ke inkubator 44°C.', 'positive-note'); requireObjects(ecbRack, incubator44); } else { setStatus(`✓ Inokulum masuk ke ECB (${state.ecbTubes.size}/6). Pilih LB positif berikutnya, pijarkan ose, lalu ulangi.`, 'positive-note'); requireObjects(bunsen, ose, ...$$('.test-tube.positive').filter(t => !state.usedLTBForECB.has(+t.dataset.index))); } return; }
      setStatus('✕ Arahkan ose ke Bunsen, LB positif, atau ECB.', 'alert');
    }
  }
  function chooseLTB(index) { const tube = $(`#tube-${index}`); if (state.stage === 5) { if (!state.positives.has(index)) { setStatus('✕ Tabung tersebut tidak menunjukkan gas Durham.', 'alert'); return; } state.selectedPositives.add(index); tube.classList.add('selected'); setStatus('✓ Tabung presumtif positif dipilih.', 'positive-note'); if (state.selectedPositives.size === state.positives.size) setTimeout(() => showPresentation(3), 400); } if (state.stage === 6 && state.positives.has(index)) { if (state.usedLTBForBGLB.has(index)) { setStatus('Tabung LB positif ini sudah dipindahkan ke BGLB.', 'alert'); return; } state.sourceTube = index; $$('.test-tube').forEach(t => t.classList.remove('selected', 'lifted')); tube.classList.add('selected', 'lifted'); setStatus('✓ Tabung LB positif diangkat. Pijarkan ose lalu ambil inokulum.', 'positive-note'); } if (state.stage === 7 && state.positives.has(index)) { if (state.usedLTBForECB.has(index)) { setStatus('Tabung LB positif ini sudah dipindahkan ke ECB.', 'alert'); return; } state.sourceTube = index; $$('.test-tube').forEach(t => t.classList.remove('selected', 'lifted')); tube.classList.add('selected', 'lifted'); setStatus('✓ Tabung LB positif diangkat. Pijarkan ose lalu ambil inokulum.', 'positive-note'); } }
  function chooseConfirm() { /* Tabung BGLB tetap dapat diangkat bebas; ECB memakai inokulum langsung dari LB positif. */ }
  ...
  function incubateBGLB() { if (!state.bglbInIncubator) { setStatus('✕ Masukkan rak BGLB ke inkubator terlebih dahulu.', 'alert'); return; } setAction('', null); setStatus('Inkubasi BGLB: 2 × 24 jam.'); countdown(2, () => { state.bglbTubes.forEach(tube => tube.classList.add('negative')); setStatus('✓ Hasil BGLB: tidak ada tabung positif / tidak terbentuk gas.', 'positive-note'); setStage(7); }); }
  function incubateECB() { const temp = Number($('#temp44').value); if (!state.ecbInIncubator) { setStatus('✕ Masukkan rak ECB ke inkubator terlebih dahulu.', 'alert'); return; } if (temp !== 44) { setStatus('✕ Atur inkubator ECB pada 44°C sebelum inkubasi.', 'alert'); return; } setAction('', null); setStatus('Inkubasi ECB 44°C: 2 × 24 jam.'); countdown(2, () => { state.ecbTubes.forEach(tube => tube.classList.add('negative')); setStatus('✓ Hasil ECB: tidak ada tabung positif / tidak terbentuk gas.', 'positive-note'); setStage(8); showPresentation(4); }); }
  ...
  function nearestTarget(x, y) {
    let candidates = []; if (state.stage === 1) candidates = [bottle]; if (state.stage === 2) candidates = $$('.test-tube'); if (state.stage === 3) candidates = [incubator]; if (state.stage === 6) candidates = [bunsen, incubator, ...$$('.test-tube.positive'), ...$$('.bglb-tube')]; if (state.stage === 7) candidates = [bunsen, incubator44, ...$$('.test-tube.positive'), ...$$('.ecb-tube')];
  ...
  function showPresentation(index = activeSlide) { activeSlide = Math.max(0, Math.min(4, index)); $$('.slide', presentation).forEach(slide => slide.classList.toggle('active', +slide.dataset.slide === activeSlide)); $('#slideCount').textContent = `${activeSlide + 1} / 5`; $('#prevSlide').disabled = activeSlide === 0; $('#nextSlide').disabled = activeSlide === 4; presentation.classList.remove('hidden'); }
